...
{
    ggplot(data = working_data, 
           aes(x = datetime, y = log10(adjusted))
           ) + 
      theme_classic() + 
      geom_line()
  }
...
# working_data2 <- working_data %>% filter(datetime <= CUT)


working_data2$adjusted -> Y_obs
working_data2$n -> X_obs

model <- nls(#Y_obs ~   ( (A + cos(X_obs/B))^(C) ) * exp(-D * X_obs) + X_obs * E, 
             Y_obs ~ (A + sin(G + X_obs/B))^C * exp(- D * X_obs) + X_obs * E, 
             start = list(A = 1, B = 210, C = 2, D = 0.0001, E = 0.00003, G = 3.14/2) 
             )

model %>% broom::tidy()

# Summary of the fitted model
summary(model)$coef[,1] -> res_optim

best_fit <- function(X, A, B, C, D, E, G){
  (A + sin(G + X/B))^C * exp(- D * X) + X * E
}

best_fitting <- best_fit(X_obs, res_optim["A"], res_optim["B"], res_optim["C"], res_optim["D"], res_optim["E"], res_optim["G"] )

ggplot(data = working_data2, aes(x = n, y = adjusted)) + 
  theme_classic() + 
  geom_line() + 
  geom_line(
    color = "red", 
    data = data.frame(n = X_obs, adjusted = best_fitting), 
    aes(x = n, y = adjusted)
  )

last_date <- max(working_data2$datetime)
last_n <- max(working_data2$n)
extend <- as.numeric(as.Date("2026-05-01") - max(working_data2$datetime)  )

new_dates <- last_date + days(c(1:extend))
new_ns <- last_n + c(1:extend)

new_extends = best_fit(X = new_ns, res_optim["A"], res_optim["B"], res_optim["C"], res_optim["D"], res_optim["E"], res_optim["G"] )
...
combined <- 
  rbind(
    working_data2 %>% 
      mutate(best_fit = 
               best_fit(X = n, res_optim["A"], res_optim["B"], res_optim["C"], res_optim["D"], res_optim["E"], res_optim["G"] )) %>% 
      select(all_of(colnames(project))
             ), 
...
ggplot(data = combined, 
  ...
  theme_classic() + 
  geom_line() + 
  geom_line(data = combined, aes(x = datetime, y = best_fit), color = "blue") + 
  
  geom_vline(linetype = "dashed", color = "darkgrey", size = 1, xintercept = DATE) +  
  geom_vline(linetype = "dashed", color = "red", size = 1, xintercept =  CUT) + 
  
  annotate("text", x=as.Date("2020-06-01"), y=10, 
           label= paste0("Projected date: ", DATE
                         )
           ) 
